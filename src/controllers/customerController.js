...
const CustomerController = { createCustomer: async (req, res) => {
 const { userId, name, email, phone, address } = req.body; if (!userId || !name || !email) {
 return res.status(400).json({ message: 'User ID, name, and email are required' });
 }
 try {
 const customerId = await CustomerModel.create(userId, name,
email, phone, address);
 ...
 getCustomerByUserId: async (req, res) => { const { userId } = req.params;
 try {
 ...
 try {
 const affectedRows = await CustomerModel.update(id, name,
email, phone, address);
 if (affectedRows === 0) {
