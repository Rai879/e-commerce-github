...
const UserController = { registerUser: async (req, res) => {
...
userId });
 ...
 loginUser: async (req, res) => { const { username, password } = req.body;
...
user.id, username: user.username, role: user.role } });
 ...
 const { username, password, role } = req.body;
 try {
 const affectedRows = await UserModel.update(id, username,
password, role);
 if (affectedRows === 0) {
